...
const CHAR_READ_RATE: float = 0.5
const PUNCTUATION_PAUSE: float = 0.05
...
func _ready() -> void:
	...
	var pause_menu = get_node_or_null("res://scenes/Task/")
...
func _reveal_text(text: String) -> void:
	...
	for i in range(text.length()):
		dialogue_label.txt = text.substr(0, i + 1)
...
func _finished_text_reveal() -> void:
